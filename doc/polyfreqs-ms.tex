...
\begin{enumerate}
  ...
  \begin{enumerate}[label={\arabic{enumi}.\arabic*.}]
    ...
    \begin{enumerate}[label={\arabic{enumi}.\arabic{enumi}.\arabic*.}]
      ...
      \item Calculate the Binomial likelihood for the simulated data for sample $m$ as the product across individuals: $\mathcal{\tilde{S}}_{\ell}^{[m]} = \sum_{i=1}^{N}P\left(\tilde{r}_{i \ell}^{[m]} | t_{i \ell},\tilde{g}_{i \ell}^{[m]},\epsilon\right)$.
      \item Calculate the Binomial likelihood for the observed data for sample $m$ as the product across individuals: $\mathcal{S}_{\ell}^{[m]} = \sum_{i=1}^{N}P\left(r_{i \ell}^{[m]} | t_{i \ell},\tilde{g}_{i \ell}^{[m]},\epsilon\right)$.
    \end{enumerate}
    \item Calculate the difference between the Binomial likelihoods for the observed read counts and the simulated read counts $\left\{ \mathcal{L}_{\ell}^{[1]}-\mathcal{\tilde{L}}_{\ell}^{[1]},\ldots,\mathcal{L}_{\ell}^{[M]}-\mathcal{\tilde{L}}_{\ell}^{[M]}\right\}$.
\end{enumerate}
\item Determine if the 95\% highest posterior density (HPD) interval of the distribution of likelihood differences contains 0.
\end{enumerate}
\medskip

When the distribution of the differences in likelihoods between the observed and simulated datasets does not contain 0 in the 95\% HPD interval, it provides evidence that the locus being examined does not follow a pattern of strict polysomic inheritance. We implement this posterior predictive model checking procedure in the \textsc{polyfreqs} package with the function \texttt{polyfreqs\_pps}.
...
\begin{figure}[b]
\centering
\caption{\textbf{Heat maps of error rates for allele frequency estimation in tetraploids}. The allele frequencies are 0.01, 0.4, 0.6 and 0.8. I need to redo these analyses and remake this figure.}
\vspace{0.25in}
\includegraphics{fig/figure1-heatmaps}
\end{figure}

\begin{figure}[b]
\centering
\caption{\textbf{Heat maps of error rates for allele frequency estimation in hexaploids}. The allele frequencies are 0.01, 0.4, 0.6 and 0.8. I need to redo these analyses and remake this figure.}
\vspace{0.25in}
\includegraphics{fig/figure2-heatmaps}
\end{figure}
...
\caption{\textbf{Heat maps of error rates for allele frequency estimation in hexaploids}. The allele frequencies are 0.01, 0.4, 0.6 and 0.8. I need to redo these analyses and remake this figure.}
\vspace{0.25in}
\includegraphics{fig/figure3-coverage}
\end{figure}
