...
\begin{enumerate}
  ...
  \begin{enumerate}[label={\arabic{enumi}.\arabic*.}]
    ...
    \begin{enumerate}[label={\arabic{enumi}.\arabic{enumi}.\arabic*.}]
      ...
      \item Calculate the Binomial likelihood for the simulated data for sample $m$ as the product across individuals: $\mathcal{\tilde{L}}_{\ell}^{[m]} = \prod_{i=1}^{N}P\left(\tilde{r}_{i \ell}^{[m]} | t_{i \ell},\tilde{g}_{i \ell}^{[m]},\epsilon\right)$.
      \item Calculate the Binomial likelihood for the observed data for sample $m$ as the product across individuals: $\mathcal{L}_{\ell}^{[m]} = \prod_{i=1}^{N}P\left(r_{i \ell}^{[m]} | t_{i \ell},\tilde{g}_{i \ell}^{[m]},\epsilon\right)$.
    \end{enumerate}
